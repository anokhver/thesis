"""Two-view augmentation pipeline + validation transform.

Lifted from ``notebooks/loaded-weiights-pretrained/pretrain_simmim_vicreg_swin.ipynb``
and parameterised; behaviour matches the original. Augmentation choices
(flips, 90 deg rotations, small translation, Poisson + Gaussian noise,
per-channel z-score) follow the fluorescence-microscopy guidance in
``.my/doc/aug``, which references the strategies used by Ma 2024 (Cell
Segmentation Challenge), CellSeg3D, Huang 2020 and Kraus 2024.
"""
...
class MicroscopyTwoViewTransform:
    """Two-view augmentation for SSL on multi-channel synaptic microscopy.

    Input  : ``(C, H, W)`` float32 tensor in ``[0, 1]``.
    Output : ``(view1, view2)`` -- each ``(C, H, W)`` float32, post-normalisation.
    """
...
class ValSingleViewTransform:
    """Validation transform: per-channel z-score, no augmentation."""
