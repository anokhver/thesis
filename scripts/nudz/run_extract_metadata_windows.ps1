param(
    ...
    [string]$OutputRoot = "Z:\anokhver_temp\Microscopy_meta",
    [string]$SmokeOutputRoot = "Z:\anokhver_temp\Microscopy_meta_smoketest",
...
if ($NoLog) {
    ...
    if ($proc.ExitCode -ne 0) {
        throw "Metadata extraction failed with exit code $($proc.ExitCode)."
    }
...
$outLog = Join-Path $logDir "extract_metadata_${Mode}_${ts}.out.log"
$errLog = Join-Path $logDir "extract_metadata_${Mode}_${ts}.err.log"
...
if ($proc.ExitCode -ne 0) {
    throw "Metadata extraction failed with exit code $($proc.ExitCode). Check logs in $logDir"
}
