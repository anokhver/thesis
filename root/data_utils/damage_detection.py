...
Detects "damaged" patches in a preprocessed dataset by comparing each patch's
intensity statistics against the per-source-image distribution.

Why per-source-image?
    The dataset contains 67+ confocal images with very different staining
    intensities. A patch that is dark relative to its siblings (same source
    image) is the right notion of "damaged" -- the floor / pedestal /
    saturation profile differs from neighbouring patches that came from the
    same microscope acquisition.

Damage criteria (flag if ANY of these triggers):
    1. base_outlier  -- |low_p5 - image_median(low_p5)| > K * MAD(low_p5)
    2. mean_outlier  -- |mean   - image_median(mean)|   > K * MAD(mean)
    3. saturated     -- mean >= saturation_thresh (e.g. >= 0.97)
    4. dead          -- max <  dead_thresh        (e.g. <  1e-4)

The MAD-based thresholds are robust to a few real outliers existing in the
group; the absolute saturation/dead thresholds catch images-level breakage
that is uniform across all patches of an image.

Outputs:
    A list of dicts with one entry per patch, ready to merge into index.csv.
    Columns added: mean, std, min, max, low_p5, high_p95, damaged (bool),
                   damage_reasons (semi-colon separated string).
"""
...
def patch_stats(
    ...
    """Compute scalar intensity stats over all channels & pixels of a patch.

    Args:
        patch: (C, H, W) float array, expected to be in [0, 1] but not
            required.
        low_p:  percentile interpreted as "base intensity / pedestal"
        high_p: percentile interpreted as "bright tail"
    Returns:
        dict with keys: mean, std, min, max, low_p{n}, high_p{n}.
    """
...
def flag_damaged(
    ...
    *,
    ...
    """Annotate each record with `damaged` (bool) and `damage_reasons` (str).

    Mutates and returns the input list. Records must already contain the
    per-patch stats produced by `patch_stats` (use `attach_stats` if not).

    The MAD-based outlier check is computed *within* each `group_key` group
    -- patches are compared to other patches from the same source image,
    not to the dataset as a whole. Single-patch groups skip the MAD check
    (no peers to compare against) and only run the absolute checks.
    """
...
def attach_stats(
    ...
    *,
    ...
    """Read each patch from disk and merge `patch_stats` into its record.

    Records must have a "filename" field. Returns the same list with stat
    keys added.
    """
    out: list[dict] = []
    for r in records:
