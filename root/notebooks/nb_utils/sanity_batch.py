"""Canonical sanity-batch indices + overfit-on-batch helper.

The same indices are imported by every training and clustering notebook so
the visualisations are directly comparable across notebooks.
"""
...
def overfit_on_batch(
    ...
    *,
    ...
    The encoder + heads are mutated *in place*. By design this does NOT
    reset the model afterwards (the user prefers to keep the warmed-up
    weights for the full training run); pass ``restore_state=True`` to
    snapshot/restore the encoder weights and RNG state instead.

    Returns a dict with the per-step history of loss / VICReg terms and
    the percentage drop in reconstruction loss.
    """
