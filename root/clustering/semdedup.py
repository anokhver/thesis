"""SemDeDup-style semantic redundancy diagnostics for patch datasets.

Identifies groups of near-duplicate patches in embedding space using the
algorithm from Abbas et al. (ICLR 2023) "SemDeDup: Data-efficient learning
at web-scale through semantic deduplication", adapted for small-to-medium
patch datasets (10K–50K patches) typical of fluorescence microscopy SSL.

Algorithm:
    1. L2-normalise embeddings (cosine ≡ inner product on the unit sphere).
    2. K-means to partition the embedding space into buckets.
    3. Within each bucket compute pairwise cosine similarities.
    4. For a sweep of dissimilarity thresholds ε, count pairs with
       cosine > 1 − ε and build a duplicate graph to find connected
       components (semantic duplicate groups).

All outputs are diagnostic — no patches are removed or flagged by default.
The user decides what action (if any) to take based on the report.

References
----------
Abbas, A. et al. (2023). SemDeDup: Data-efficient learning at web-scale
    through semantic deduplication. arXiv:2303.09540.
    - k-means bucketing, within-cluster cosine, ε-sweep, greedy retention.
    - Sec 3: algorithm; Sec 6.1: k is robust; Sec 6.3: which duplicate
      to keep doesn't matter; Table 5: overhead <1% of training cost.
Caron, M. et al. (2018). Deep Clustering for Unsupervised Learning of
    Visual Features. ECCV.
    - PCA-256 + whiten + L2 before k-means (Sec 3.4). We skip whitening
      to preserve cosine geometry (rubber-duck review).
"""
...
def _within_cluster_cosines(
    ...
    """Compute upper-triangle pairwise cosines within one cluster.

    Subsamples if the cluster is larger than ``max_size``.
    Returns a 1D array of cosine values.
    """
...
def _build_duplicate_graph(
    ...
    """Build a graph of duplicate pairs within each cluster.
...
def _same_vs_cross_image_cosines(
    ...
    """Sample cosine similarities for same-image vs cross-image patch pairs.

    Helps calibrate ε thresholds for this specific dataset.
    """
...
def redundancy_profile(
    ...
    """Run the full SemDeDup-style redundancy diagnostic.

    Parameters
    ----------
    Z : (N, D) float32
        Patch embeddings (raw encoder output, NOT PCA-whitened).
    source_images : (N,) str
        Source image filename for each patch.
    cfg : SemDeDupCfg, optional
        Configuration. Defaults are sensible for 10K–50K patches.

    Returns
    -------
    RedundancyReport
        Diagnostic report with per-threshold stats, per-cluster stats,
        cosine histograms, and per-image duplicate contribution.
    """
    if cfg is None:
        cfg = SemDeDupCfg()
...
def print_summary(report: RedundancyReport):
    """Print a compact text summary of the redundancy report."""
