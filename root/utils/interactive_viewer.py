...
class VSIBrowser:
    """Interactive browser for VSI microscopy files with thumbnail preview and 3D visualization."""
    ...
    def load_thumbnail(self, file_path: Path, target_size: int = 256) -> NDArray:
        """Load VSI as regular image for fast preview without microscopy reader.

        Why this works: VSI files have an embedded preview that PIL can read directly,
        avoiding the overhead of parsing the full microscopy metadata.
        """
        try:
    ...
    def visualize(self, idx: int) -> None:
        ...
        from data_utils.load_data import load_full_volume, interpolate_z_axis
