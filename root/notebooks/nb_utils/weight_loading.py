"""Pretrained-weight loading: timm Swin -> MONAI SwinTransformer + local.

The timm -> MONAI key remap encodes the structural correspondence between
the two implementations:
- timm ``layers.{i}.blocks.*``       <->  MONAI ``layers{i+1}.0.blocks.*``
- timm ``layers.{i}.downsample.*``   <->  MONAI ``layers{i}.0.downsample.*``
- timm ``mlp.fc1`` / ``mlp.fc2``     <->  MONAI ``mlp.linear1`` / ``linear2``

Reference implementations consulted:
- timm Swin model: https://github.com/huggingface/pytorch-image-models
  (``timm/models/swin_transformer.py``).
- MONAI SwinUNETR / SwinTransformer:
  https://github.com/Project-MONAI/MONAI (``monai/networks/nets/swin_unetr.py``).

The first-conv channel-count adaptation (average source channels and
tile to the target, scaled by ``src/target``) is the standard transfer
trick used by timm ``_convert_in_chans`` and MONAI's pretrained loaders
to adapt 3-channel ImageNet weights to other input dimensionalities.
"""
...
def _adapt_first_conv(weight: torch.Tensor, target_in_chans: int) -> torch.Tensor:
    """Average-then-tile patch_embed.proj.weight to a different ``in_chans``."""
...
def _load_moby_ckpt(
    ...
    """Load a MoBY checkpoint and remap to MONAI naming.

    MoBY (Xie et al., 2021) saves the full model (encoder + projector +
    contrastive head) under the ``model`` key.  Backbone weights are
    prefixed with ``encoder.``; this function strips that prefix (and
    the DDP ``module.`` prefix if present), drops non-backbone keys,
    then remaps via ``convert_timm_to_swinunetr_state_dict``.
    """
...
def load_pretrained_into_encoder(
    ...
    *,
    ...
    """Dispatch on ``base_cfg.init_source`` and load weights into ``encoder``.

    Recognised values:
      - ``"scratch"``        : no-op.
      - ``"timm_imagenet"``  : download timm and remap.
      - ``"moby"``           : load MoBY (MoCo+BYOL) checkpoint and remap.
      - ``"local_ckpt"``     : load ``base_cfg.pretrained_ckpt_path``.
    """
    log = (logger.info if logger is not None else print)
    if base_cfg.init_source == "scratch":
        log(f"[init] scratch -- {len(encoder.state_dict())} params kept random")
        return {
            "source": "scratch",
            "n_loaded": 0,
            "n_random_init": len(encoder.state_dict()),
            "n_target_params": len(encoder.state_dict()),
            "n_shape_mismatch": 0,
